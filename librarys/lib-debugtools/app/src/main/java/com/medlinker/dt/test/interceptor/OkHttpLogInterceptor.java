...
public class OkHttpLogInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (isShowFullLog) {
            ...
            if (requestBody == null) {
                Log.i(TAG, "Body - no body");
            } else {
                Buffer buffer = new Buffer();
                requestBody.writeTo(buffer);
                try {
                    Log.i(TAG, "Body - " + buffer.readString(requestBody.contentType().charset()));
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
        ...
        if (isShowFullLog) {
            Log.i(TAG, "<--- RESPONSE " + response.code() + " " + response.request().url());
        ...
        try {
            ...
            if (TextUtils.isEmpty(responseBody)) {
                Log.i(TAG, request.method() + " " + request.url() + "Body - no body");
            } else {
                Log.i(TAG, request.method() + " " + request.url() + "Body - " + " \n" + json.toString(2));
            }
        ...
        if (body != null) {
            Log.i(TAG, "<--- END " + "(Size: " + body.contentLength() + " bytes - " + "Network time: " + (t2 - t1)
                    / MILLI_AS_NANO + " ms)");
        }
    ...
    private static void logHeaders(Headers headers) {
        for (String headerName : headers.names()) {
            for (String headerValue : headers.values(headerName)) {
                Log.i(TAG, "Header - [" + headerName + ": " + headerValue + "]");
            }
        }
    }
