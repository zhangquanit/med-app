...
//            if (!Warehouse.groupsIndex.containsKey(postcard.getGroup())) {
            if (!Warehouse.containsGroup(postcard.getGroup())) {
    ...
    public synchronized static void addRouteGroupDynamic(String groupName, IRouteGroup group) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        //Med-Arouter
        if (Warehouse.containsGroup(groupName)) {
            ...
            for (Map.Entry<String, Class<? extends IRouteGroup>> entry : Warehouse.groupsIndex.entrySet()) {
                ...
                if (key.contains("@@")) {
                    ...
                    if (android.text.TextUtils.equals(itemGroup, groupName)) {
                        groupList.add(itemGroup);
                        entry.getValue().getConstructor().newInstance().loadInto(Warehouse.routes);
                    }
                }
            }
            for (String item : groupList) {
                Warehouse.groupsIndex.remove(item);
            }
        }
