...
public class Constants {
    ...
    public static String getImReceiverAction(Context context) {
        return String.format(IM_RECEIVER_ACTION, context.getPackageName());
    }

    public static String getReceiverPermission(Context context) {
        return String.format(RECEIVER_PERMISSION, context.getPackageName());
    }

    /**
     * 广播action
     */
    private static final String IM_RECEIVER_ACTION = "android.intent.action.im.receiver_action.%s"; // 如果与医联app同名，IM消息会串

    /**
     * 自定义权限
     */
    private static final String RECEIVER_PERMISSION = "android.intent.permission.im.%s_receiver_permission";
