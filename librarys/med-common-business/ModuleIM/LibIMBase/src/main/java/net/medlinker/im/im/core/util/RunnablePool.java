...
import net.medlinker.im.im.util.Cacher;
...
public final class RunnablePool {
    ...
    public static class Runner implements Runnable {
        ...
        public void run() {
            ...
            if (executor == null) {
                LogUtil.w("RunnablePool_Runner", "run", "mExecutor == null or is recycled(Fragment or Activity)");
                return;
            }
            ...
            if (executor instanceof Activity) {
                if (((Activity) executor).isFinishing()) {
                    LogUtil.i("RunnablePool_Runner", "run", "executor is Activity and isFinishing() = true. ");
                    shouldExecute = false;
                }
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR1 && ((Activity) executor).isDestroyed()) {
                    LogUtil.i("RunnablePool_Runner", "run", "executor is Activity and isDestroyed() = true. ");
                    shouldExecute = false;
                }
                ...
                if (((Fragment) executor).isDetached() || ((Fragment) executor).isRemoving()) {
                    ...
                    LogUtil.i("RunnablePool_Runner", "run", "executor is Fragment and isDestroyed() ||  isRemoving() = true. ");
                    shouldExecute = false;
                }
                ...
                if (((android.app.Fragment) executor).isDetached() || ((android.app.Fragment) executor).isRemoving()) {
                    // ((Fragment) executor).isVisible()
                    LogUtil.i("RunnablePool_Runner", "run", "executor is android.app.Fragment and isDestroyed() ||  isRemoving() = true. ");
                    shouldExecute = false;
                }
            }
            if (shouldExecute) {
