...
public class RoundCornerTextView extends AppCompatTextView {
    ...
    protected void dealTypeArray(TypedArray typedArray) {
        if (typedArray == null) {
            return;
        }
        shape = typedArray.getInt(R.styleable.RoundCornerTextView_shape, RECTANGLE);
        mScaleType = typedArray.getInt(R.styleable.RoundCornerTextView_scaledType, FIT_CENTER);
        mBackgroundFitType = typedArray.getInt(R.styleable.RoundCornerTextView_backgroundFitType, BACK);
        stroke_colors = typedArray.getColorStateList(R.styleable.RoundCornerTextView_strokeColor);
        solid_colors = typedArray.getColorStateList(R.styleable.RoundCornerTextView_solidColor);
        stroke_width = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_strokeWidth, 0);
        self_round_corner = typedArray.getBoolean(R.styleable.RoundCornerTextView_selfRoundCorner, false);
        if (self_round_corner) {
            corner_radius = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_selfRadius, 0);
            radius_left_top = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_radius_left_top, -1);
            if (radius_left_top < 0) {
                radius_left_top = corner_radius;
            }
            radius_right_top = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_radius_right_top, -1);
            if (radius_right_top < 0) {
                radius_right_top = corner_radius;
            }
            radius_right_bottom = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_radius_right_bottom, -1);
            if (radius_right_bottom < 0) {
                radius_right_bottom = corner_radius;
            }
            radius_left_bottom = typedArray.getDimensionPixelSize(R.styleable.RoundCornerTextView_radius_left_bottom, -1);
